...
export const createScheduledContent = async (data) => {
  try {
    const code = await generateUniqueCode();

    // Content expires 7 days after unlock time
    const expiresAt = new Date(
      data.unlockAt.getTime() + 7 * 24 * 60 * 60 * 1000
    );
    ...
    logger.info(
      `Scheduled content created: ${code} (unlocks: ${data.unlockAt.toISOString()})`
    );
...
export const retrieveScheduledContent = async (code) => {
  try {
