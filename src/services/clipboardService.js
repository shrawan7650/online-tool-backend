...
import { ClipboardNote } from '../models/ClipboardNote.js';
...
const decryptText = (textEnc, iv, authTag) => {
...
export const createClipboardNote = async (data) => {
  try {
    ...
    let pinHash;
    if (data.pin) {
      const saltRounds = parseInt(process.env.PIN_ROUNDS || '10', 10);
    ...
    logger.info(
      `Clipboard note created: ${code} (expires: ${expiresAt.toISOString()})`
    );
...
export const retrieveClipboardNote = async (code, pin) => {
  try {
    ...
    logger.info(
      `Clipboard note retrieved: ${code} (${updatedNote.remainingReads} reads remaining)`
    );
...
export const deleteClipboardNote = async (code) => {
  try {
